...
printf '[xray] building layer causal sensitivity probe for sm_%s\n' "$CC"
"$NVCC" --threads=0 --use_fast_math -std=c++17 -O3 \
  --generate-code "arch=compute_${CC},code=sm_${CC}" \
  -I. dev/xray/layer_sensitivity_probe.cu \
  -lcublas -lcublasLt \
...
printf '[xray] building token-level output sensitivity probe for sm_%s\n' "$CC"
"$NVCC" --threads=0 --use_fast_math -std=c++17 -O3 \
  --generate-code "arch=compute_${CC},code=sm_${CC}" \
  -I. dev/xray/output_sensitivity_probe.cu \
  -lcublas -lcublasLt \
  -o xray_output_sensitivity_probe
...
printf '\n[xray] token-level output consequence discovery\n'
./xray_output_sensitivity_probe "$B" "$T"
