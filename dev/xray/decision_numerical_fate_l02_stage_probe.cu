...
static void xray_capture_l02_gpu_prefix(XrayL02GpuPrefix* p, GPT2* model,
    ...
    const int l = 2;
    const size_t BTC = (size_t)model->batch_size * T * C;
    ...
    xray_copy_gpu_prefix(p->qkv, model->acts.qkvr + (size_t)l * model->batch_size * T * 3 * C, b, T, P, 3 * C);
    ...
    xray_copy_gpu_prefix(p->fch, model->acts.fch + (size_t)l * model->batch_size * T * 4 * C, b, T, P, 4 * C);
    xray_copy_gpu_prefix(p->gelu, model->acts.fch_gelu + (size_t)l * model->batch_size * T * 4 * C, b, T, P, 4 * C);
...
int main(int argc, char** argv) {
    ...
    printf("[xray][decision-l02-stage] target=%d b=%d t=%d ref=%d low=%d gpu_low_pair=%+.9e\n",
           target,b,t,ref_winner,low_winner,low_margin);
